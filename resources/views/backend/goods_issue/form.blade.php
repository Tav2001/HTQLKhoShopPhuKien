<form role="form" action="" method="POST">
    ...
    <div class="box-body">
        <div class="form-group {{ $errors->has('w_code') ? 'has-error' : '' }}">
            ...
            <div class="col-sm-12" style="display: inline-block; padding: 0px;">
                <input class="form-control random_code" id="random_code" oninput="if(value.length>15)value=value.slice(0,15)" name="w_code" value="{{ old('w_code', isset($warehousing) ? $warehousing->w_code : '') }}" type="text" placeholder="Mã đơn hàng (NH_)" required>
            </div>
            ...
            <tbody class="content-table">
                @if (!isset($warehousing))
                    <tr location="0" class="product_0">
                        ...
                        <td>
                            <input type="number" class="form-control pw_total_number" name="pw_total_number[0]" value="" placeholder="Số lượng" min="1" required>
                        </td>
                        <td>
                            <span style="display:none"><input type="radio"  value="1" name="pw_active_price[0]" checked class="pw_active_price"> Giá nhập</span>
                            <input type="radio" value="4" name="pw_active_price[0]" class="pw_active_price"> Giá khác
                            <input type="number" class="form-control mg-t-10 pw_custom_price" oninput="if(value.length > 8 )value=value.slice(0, 15)" name="pw_custom_price[0]" value="" placeholder="Giá hàng hóa" style="display: none">
                        </td>
                ...
                @else
                    @foreach($warehousing->warehousingProduct as $keys => $productData)
                        <tr location="{{ $keys }}" class="product_{{ $keys }}">
                            ...
                            <td>
                                <input type="number" class="form-control pw_total_number" name="pw_total_number[{{ $keys }}]" value="{{ isset($productData) ? $productData->pw_total_number : '' }}" placeholder="Số lượng" required>
                            </td>
                            <td>
                                <span style="display:none"><input type="radio" value="1" name="pw_active_price[{{ $keys }}]" {{ $productData->pw_active_price == 1 ? 'checked' : '' }}  class="pw_active_price"> Giá nhập </span>
                                <input type="radio" value="4" name="pw_active_price[{{ $keys }}]" {{ $productData->pw_active_price == 4 ? 'checked' : '' }} class="pw_active_price"> Giá khác
                                <input type="number" class="form-control mg-t-10 pw_custom_price" oninput="if(value.length > 8 )value=value.slice(0, 15)" name="pw_custom_price[0]" value="{{ $productData->pw_custom_price }}" placeholder="Giá hàng hóa" style="display: {{ $productData->pw_active_price == 4 ? 'block' : 'none' }}">
                            </td>
    ...
    <div class="box-body text-right">
        ...
        @endif -->
        <button type="button" class="btn btn-success mg-t-20 mg-b-15 btn-add-row-import_product"><i class="fa fa-plus-circle"></i> Thêm</button>
    </div>
    <!-- /.box-body -->
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-center">
                ...
                <h4 class="modal-title" style="font-weight: bold; font-size: 20px;">Import Excel Danh Sách hàng hóa </h4>
            </div>
