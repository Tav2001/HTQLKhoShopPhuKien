<form role="form" action="" method="POST" enctype="multipart/form-data">
    ...
    <div class="box-body">
        <div class="form-group {{ $errors->has('p_code') ? 'has-error' : '' }}">
            <div class="fs-13">
                <label for="company">Mã hàng hóa <span class="title-sup">(*)</span></label>
            </div>
            <div class="col-sm-12" style="display: inline-block; padding: 0px;">
                <input class="form-control random_code" id="random_code" oninput="if(value.length>15)value=value.slice(0,15)" name="p_code" value="{{ old('p_code', isset($product) ? $product->p_code : '') }}" type="text" placeholder="Mã hàng hóa">
            </div>
        ...
        <div class="form-group {{ $errors->has('p_name') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1" class="mg-t-10">Tên hàng hóa <sup class="title-sup">(*)</sup></label>
            <input type="text" class="form-control" name="p_name" value="{{ old('p_name', isset($product) ? $product->p_name : '') }}" placeholder="Tên hàng hóa">
        ...
        <div class="form-group {{ $errors->has('p_category_id') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Loại hàng hóa <sup class="title-sup">(*)</sup></label>
        ...
        <div class="form-group {{ $errors->has('p_retail_price') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Giá bán lẻ hàng hóa</label>
            <input type="number" class="form-control" name="p_retail_price" value="{{ old('p_retail_price', isset($product) ? $product->p_retail_price : '') }}" oninput="if(value.length > 15 )value=value.slice(0, 15)" placeholder="Giá bán lẻ hàng hóa">
        ...
        <div class="form-group {{ $errors->has('p_cost_price') ? 'has-error' : '' }}">
            <label for="exampleInputEmail1">Giá bán sỉ hàng hóa</label>
            <input type="number" class="form-control" name="p_cost_price" value="{{ old('p_cost_price', isset($product) ? $product->p_cost_price : '') }}" oninput="if(value.length > 15 )value=value.slice(0, 15)" placeholder="Giá bán sỉ hàng hóa">
        ...
        <div class="form-group mg-t-20">
            <label for="inputEmail3">Ảnh hàng hóa </label>
